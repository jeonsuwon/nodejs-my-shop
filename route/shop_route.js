...
router.post('/shop', async (req, res) => {
  ...
  if (!goodsName) {
    return res.status(401).json({ errorMessage: "데이터가 존재하지 않습니다." });
  }
    ...
    const shop = new Shop({ goodsNumber, goodsName, goodsInfor, manager, PW, status, creatDate });
    await shop.save();
...
router.patch('/shop/:shopId', async (req, res) => {
  ...
  if (currentshop.PW == PW) {
    if (goodsName && goodsInfor && manager) {
      ...
      return res.status(404).json({ errorMessage: "상품명, 정보, 담당자명을 입력해주세요." })
    }
  } else {
